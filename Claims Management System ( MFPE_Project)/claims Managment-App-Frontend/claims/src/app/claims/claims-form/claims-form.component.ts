...
export class ClaimsFormComponent implements OnInit {
  ...
  fetchStatus(claimID: string, policyID:number, memberID:string) {
    this.isValidating = true;
    this.claimsService.viewStatus({ claimID,policyID,memberID}).subscribe(
      (response) => {
        this.isValidating = false;
  ...
  onSubmit(form: NgForm) {
    ...
    if (this.isBillMode) {
      ...
      this.fetchStatus(form.value.claimID,form.value.policyID,form.value.memberID);
    }
